# Experiment report: static example screening

Last updated: 2026-07-11

Status: completed screening record; some publication decisions below have since
been superseded.

This report records the evidence-based screen used to choose static Audio
Jacobian Lens examples. The goal was not to collect outputs that merely look
fluent. Each example had to teach one specific fact about an intermediate
readout, an actual output head, a limitation, or a causal intervention.

The current public release contains the ASR and speech-to-speech explorers.
Chatterbox TTS was retired from public navigation because its pilot is not yet
interpretable enough for the main story. ASR Audio 10 is now the attributed
Bosker Laurel/Yanny clip and includes the recorded steering replay; the
standalone steering route is only a compatibility redirect. Treat the older
three-family recommendations below as the historical screening record, not the
current route contract. See [`../PUBLISHING.md`](../PUBLISHING.md) for that
contract.
...
## Historical Chatterbox/TTS screen (not public)
...
## Follow-up status at the time of screening
